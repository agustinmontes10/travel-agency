"use client";

import type { ReactNode } from "react";
import Link from "next/link";
import { motion } from "motion/react";
import { track } from "@vercel/analytics";

interface HeroContentProps {
  whatsappUrl: string;
}

const EASE = [0.16, 1, 0.3, 1] as const;

/**
 * Línea de titular con "mask reveal": el texto sube desde detrás de una
 * máscara invisible. El padding evita que se recorten los descendentes
 * de la tipografía serif en cursiva.
 */
function MaskedLine({ children, delay }: { children: ReactNode; delay: number }) {
  return (
    <span className="block overflow-hidden pb-[0.14em] -mb-[0.14em]">
      <motion.span
        className="block"
        initial={{ y: "115%" }}
        animate={{ y: "0%" }}
        transition={{ duration: 1.1, ease: EASE, delay }}
      >
        {children}
      </motion.span>
    </span>
  );
}

export function HeroContent({ whatsappUrl }: HeroContentProps) {
  return (
    <div className="mx-auto flex max-w-4xl flex-col items-center gap-7 text-center">
      <motion.p
        className="flex items-center gap-4 text-[0.65rem] font-semibold uppercase tracking-[0.32em] text-sand sm:text-xs"
        initial={{ opacity: 0, y: 16 }}
        animate={{ opacity: 1, y: 0 }}
        transition={{ duration: 0.7, ease: EASE, delay: 0.15 }}
      >
        <motion.span
          className="h-px w-8 origin-right bg-sand/60 sm:w-12"
          aria-hidden
          initial={{ scaleX: 0 }}
          animate={{ scaleX: 1 }}
          transition={{ duration: 0.9, ease: EASE, delay: 0.35 }}
        />
        Agencia de viajes · Gonzales Chaves
        <motion.span
          className="h-px w-8 origin-left bg-sand/60 sm:w-12"
          aria-hidden
          initial={{ scaleX: 0 }}
          animate={{ scaleX: 1 }}
          transition={{ duration: 0.9, ease: EASE, delay: 0.35 }}
        />
      </motion.p>

      <h1
        className="font-display text-5xl leading-[1.06] tracking-tight text-balance sm:text-6xl lg:text-7xl"
        style={{ textShadow: "0 2px 30px rgba(0,0,0,0.45)" }}
      >
        <MaskedLine delay={0.3}>El mundo te espera,</MaskedLine>
        <MaskedLine delay={0.45}>
          <em className="italic text-sand">nosotros te llevamos.</em>
        </MaskedLine>
      </h1>

      <motion.p
        className="max-w-xl text-sm leading-relaxed text-white/90 sm:text-base"
        style={{ textShadow: "0 1px 14px rgba(0,0,0,0.55)" }}
        initial={{ opacity: 0, y: 24 }}
        animate={{ opacity: 1, y: 0 }}
        transition={{ duration: 0.8, ease: EASE, delay: 0.75 }}
      >
        Paquetes nacionales e internacionales con asesoramiento personalizado,
        antes, durante y después de tu viaje.
      </motion.p>

      <motion.div
        className="mt-2 flex flex-col items-center gap-4 sm:flex-row"
        initial={{ opacity: 0, y: 20 }}
        animate={{ opacity: 1, y: 0 }}
        transition={{ duration: 0.8, ease: EASE, delay: 0.9 }}
      >
        <Link
          href="#packages"
          className="inline-flex h-12 items-center justify-center rounded-full bg-background px-8 text-sm font-semibold tracking-tight text-navy-deep shadow-lg shadow-black/25 transition-all duration-200 hover:-translate-y-0.5 hover:bg-sand active:scale-[0.98]"
        >
          Ver paquetes disponibles
        </Link>
        <Link
          href={whatsappUrl}
          target="_blank"
          onClick={() => track("whatsapp_click", { source: "hero" })}
          className="inline-flex h-12 items-center justify-center gap-2.5 rounded-full bg-[#25D366] px-7 text-sm font-semibold tracking-tight text-white shadow-lg shadow-black/25 transition-all duration-200 hover:-translate-y-0.5 hover:bg-[#1ebe5d] active:scale-[0.98]"
        >
          <svg viewBox="0 0 24 24" className="h-4 w-4 fill-current" xmlns="http://www.w3.org/2000/svg" aria-hidden>
            <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 0 1-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 0 1-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 0 1 2.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0 0 12.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 0 0 5.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 0 0-3.48-8.413Z" />
          </svg>
          Hablar con una asesora
        </Link>
      </motion.div>
    </div>
  );
}
